...
def get_data_to_csv(csvfile, urlbase):
    ...
    with open((csvfolder + csvfile), 'wb') as f:
        f.write(r.content)
    #making backup
    with open((csvfolder + ('full_' + csvfile)), 'wb') as f:
         f.write(r.content)
...
def get_data():    
    ...
    country = ['US']
    #6/27/2020
    #save incoming data with all columns first
    col_list = ['Country_Region', 'Province_State', 'Last_Update', 'Deaths', 'Mortality_Rate']
    df1 = pd.read_csv((csvfolder + yesterdayfile), encoding = 'latin1', usecols=col_list)
    ...
    #6/27
    col_list = ['Country_Region', 'Province_State', 'Last_Update', 'Deaths', 'Mortality_Rate']
    df_previous = pd.read_csv((csvfolder + weekagofile), encoding = 'latin1', usecols=col_list)
    df_previous = df_previous.set_index('Province_State')

    #6/27
    writelog('these are all the columns - current full')
    #workfolder + ('full_' + csvfile)
    df_full = pd.read_csv((csvfolder + ('full_' + yesterdayfile)), encoding = 'latin1')
    ...
    writelog('these are the columns - current after adding Population_2018')
    ...
    df_previous['Deaths_After'] = df['Deaths']
    df_previous['Mortality_Rate_After'] = df['Mortality_Rate']
    ...
    df_previous.to_csv(csvfolder + yesterdayfile + '-Differences_Report.csv')
    ...
    webpage = webfolder + str(today) + yesterdayfile + '-difference.html'
    with open(webpage, 'wt') as f:
        f.write(html)
